...
{
    public partial class POS_IO_M002 : FormBase, IIOM002View
    {
        #region 변수
        ...
        /// 중간입금 비즈니스 로직
        /// </summary>
        private IIOM002presenter m_Presenter;

        /// <summary>
        /// 중간입금 신규 또는 수정여부(true : 신규, false : 수정)
        /// </summary>
        ...
        void form_Load(object sender, EventArgs e)
        {
            ...
            */
            ...
            //중간입금 설정------------------------------
            m_Presenter = new IOM002presenter(this);
            
            // Loc changed 12.07
            //m_Presenter.GetTicketTitle();       //타사 상품권명 조회
            DisableFocus();

            m_Presenter.GetMiddleDeposit();     //중간입금 금액 조회
            //-------------------------------------------
            ...
            else if (e.Key.OPOSKey == WSWD.WmallPos.FX.Shared.OPOSMapKeys.KEY_ENTER)
            {
                ...
                else
                {
                    if (txtCnt.Text.Trim().Length <= 0)
                    {
                        ...
                        _iNewCnt = POSCommon.Instance.GetLabelInt(lblMidCnt) > 0 ? POSCommon.Instance.GetLabelInt(lblMidCnt) + 1 : 1;
                        if (_iNewCnt <= 5)
                        {
                            string[] strBtnNm = new string[2];
                            strBtnNm[0] = "예";
                            strBtnNm[1] = "아니오";
                            if (ShowMessageBox(MessageDialogType.Question, "", SetMsg(_iNewCnt, WSWD.WmallPos.FX.Shared.ConfigData.Current.SysMessage.GetMessage("00254")), strBtnNm) == DialogResult.Yes)
                            {
                                _bNew = true;
                                iMiddleNo = TypeHelper.ToInt32(_iNewCnt);

                                //컨트롤 초기화
                                SetInitControl(false);
                                txtCnt.Text = _iNewCnt.ToString();
                                txtCnt.Focusable = false;
                            }
                            else
                            {
                                txtCnt.Text = "";
                        ...
                        {
                            string strTempMsg = string.Format("{0}", strMsg01);
                            //오류메시지박스
                            ShowMessageBox(WSWD.WmallPos.FX.Shared.MessageDialogType.Information, null, strTempMsg);
                        }
                    }
                    else
                    {
                        _iTemp = POSCommon.Instance.GetInputTextInt(txtCnt);
                        _iTempTotalCnt = POSCommon.Instance.GetLabelInt(lblMidCnt);

                        if (_iTemp > 0 && _iTemp <= 5)
                        {
                            if (_iTempTotalCnt <= 0)
                            {
                                txtCnt.Text = "";
                                ShowMessageBox(WSWD.WmallPos.FX.Shared.MessageDialogType.Information, null, strMsg02);
                                e.IsHandled = true;
                            }
                            else
                            {
                                if (_iTemp > _iTempTotalCnt)
                                {
                                    txtCnt.Text = "";
                                    ShowMessageBox(WSWD.WmallPos.FX.Shared.MessageDialogType.Information, null, strMsg02);
                                    e.IsHandled = true;
                                ...
                                else
                                {
                                    ...
                                    strBtnNm[0] = "예";
                                    strBtnNm[1] = "아니오";
                                    if (ShowMessageBox(MessageDialogType.Question, null, SetMsg(_iTemp, WSWD.WmallPos.FX.Shared.ConfigData.Current.SysMessage.GetMessage("00255")), strBtnNm) == DialogResult.Yes)
                                    {
                                        _bNew = false;
                                        iMiddleNo = TypeHelper.ToInt32(_iTemp);
        ...
        void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                //신규 or 수정
                if (!_bNew && iMiddleNo <= 0)
                {
                    SetControlDisable(false);
                    ShowMessageBox(MessageDialogType.Warning, null, strMsgErr_02);
                    return;
                }
                ...
                m_Presenter.SetMiddelAmt(_bNew ? NetCommConstants.INPUT_INSERT : NetCommConstants.INPUT_UPDATE, 
                    iMiddleNo.ToString(), dsControl);
            }
            catch (Exception ex)
            {
        ...
        public void SetMiddleDeposit(DataSet ds)
        {
            ...
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0] != null)
            {
                ...
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    Int64 iTempCash = TypeHelper.ToInt64(dr["AM_ITEM"] != null && dr["AM_ITEM"].ToString() != "" ? dr["AM_ITEM"] : "0");

                    switch (dr["ID_ITEM"].ToString())
                    {
                        case "E00": //중간입금 총회차
                            lblMidCnt.Text = dr["CT_ITEM"] != null ? dr["CT_ITEM"].ToString() : "";
                            iMiddleNo = TypeHelper.ToInt32(dr["CT_ITEM"] != null && dr["CT_ITEM"].ToString() != "" ? dr["CT_ITEM"].ToString() : "0");
                            break;
                        case "E01": //중간입금 현금 및 상품권 01회차 금액
                        case "E21":
                            lblMidAmt01.Text = string.Format("{0:#,##0}", iTempCash + POSCommon.Instance.GetLabelInt(lblMidAmt01));
                            iTotalAmt += iTempCash;
                            break;
                        case "E02": //중간입금 현금 및 상품권 02회차 금액
                        case "E22":
                            lblMidAmt02.Text = string.Format("{0:#,##0}", iTempCash + POSCommon.Instance.GetLabelInt(lblMidAmt02));
                            iTotalAmt += iTempCash;
                            break;
                        case "E03": //중간입금 현금 및 상품권 03회차 금액
                        case "E23":
                            lblMidAmt03.Text = string.Format("{0:#,##0}", iTempCash + POSCommon.Instance.GetLabelInt(lblMidAmt03));
                            iTotalAmt += iTempCash;
                            break;
                        case "E04": //중간입금 현금 및 상품권 04회차 금액
                        case "E24":
                            lblMidAmt04.Text = string.Format("{0:#,##0}", iTempCash + POSCommon.Instance.GetLabelInt(lblMidAmt04));
                            iTotalAmt += iTempCash;
                            break;
                        case "E05": //중간입금 현금 및 상품권 05회차 금액
                        case "E25":
                            lblMidAmt05.Text = string.Format("{0:#,##0}", iTempCash + POSCommon.Instance.GetLabelInt(lblMidAmt05));
                            iTotalAmt += iTempCash;
                            break;
                        default:
                            break;
                ...
                //중간 입금 총금액
                lblMidAmtTotal.Text = string.Format("{0:#,##0}", iTotalAmt);
            }
        }
        ...
        private void SetInitControl(bool bLoad)
        {
            ...
            txtCashCnt01.ReadOnly = bLoad;
            txtCashCnt02.ReadOnly = bLoad;
            txtCashCnt03.ReadOnly = bLoad;
            txtCashCnt04.ReadOnly = bLoad;
            txtCashCnt05.ReadOnly = bLoad;
            txtCashCnt06.ReadOnly = bLoad;
            txtCashCnt07.ReadOnly = bLoad;
            txtCashCnt08.ReadOnly = bLoad;
            txtCashCnt09.ReadOnly = bLoad;

            txtCashAmt01.ReadOnly = bLoad;

            txtTicketCnt01.ReadOnly = bLoad;
            txtTicketCnt01.Focusable = !bLoad;

            txtTicketCnt02.ReadOnly = bLoad;
            txtTicketCnt02.Focusable = !bLoad;

            txtTicketCnt03.ReadOnly = bLoad;
            txtTicketCnt03.Focusable = !bLoad;

            /* Loc changed 12.07
            fc = txtTicketCnt04.Focusable;
            txtTicketCnt04.ReadOnly = bLoad;
            txtTicketCnt04.Focusable = fc;

            fc = txtTicketCnt05.Focusable;
            txtTicketCnt05.ReadOnly = bLoad;
            txtTicketCnt05.Focusable = fc;

            fc = txtTicketCnt06.Focusable;
            txtTicketCnt06.ReadOnly = bLoad;
            txtTicketCnt06.Focusable = fc;

            fc = txtTicketCnt07.Focusable;
            txtTicketCnt07.ReadOnly = bLoad;
            txtTicketCnt07.Focusable = fc;

            fc = txtTicketCnt08.Focusable;
            txtTicketCnt08.ReadOnly = bLoad;
            txtTicketCnt08.Focusable = fc;*/

            txtTicketAmt01.ReadOnly = bLoad;
            txtTicketAmt01.Focusable = !bLoad;

            txtTicketAmt02.ReadOnly = bLoad;
            txtTicketAmt02.Focusable = !bLoad;

            txtTicketAmt03.ReadOnly = bLoad;
            txtTicketAmt03.Focusable = !bLoad;

            /*
            fc = txtTicketAmt04.Focusable;
            txtTicketAmt04.ReadOnly = bLoad;
            txtTicketAmt04.Focusable = fc;

            fc = txtTicketAmt05.Focusable;
            txtTicketAmt05.ReadOnly = bLoad;
            txtTicketAmt05.Focusable = fc;

            fc = txtTicketAmt01.Focusable;
            txtTicketAmt01.ReadOnly = bLoad;
            txtTicketAmt01.Focusable = fc;

            fc = txtTicketAmt07.Focusable;
            txtTicketAmt07.ReadOnly = bLoad;
            txtTicketAmt07.Focusable = fc;

            fc = txtTicketAmt08.Focusable;
            txtTicketAmt08.ReadOnly = bLoad;
            txtTicketAmt08.Focusable = fc;*/
        ...
        {
            string strTempTotalAmt = string.Format("{0:#,##0}", POSCommon.Instance.GetInputText(txtTotalAmt));
            switch (TypeHelper.ToInt32(iMiddleNo))
            {
                case 1:
            ...
            Int64 iTempTotal = 0;
            iTempTotal += POSCommon.Instance.GetLabelInt64(lblMidAmt01);
            iTempTotal += POSCommon.Instance.GetLabelInt64(lblMidAmt02);
            iTempTotal += POSCommon.Instance.GetLabelInt64(lblMidAmt03);
            iTempTotal += POSCommon.Instance.GetLabelInt64(lblMidAmt04);
            iTempTotal += POSCommon.Instance.GetLabelInt64(lblMidAmt05);
            lblMidAmtTotal.Text = string.Format("{0:#,##0}", iTempTotal);
        }

        /// <summary>
        /// 중간입금 저장 확인후 프린팅
        /// </summary>
        /// <param name="basketHeader">중간입금 헤더정보</param>
        /// <param name="basketMiddleDeposit">중간입금 정보</param>
        public void SetTran(BasketHeader basketHeader, BasketMiddleDeposit basketMiddleDeposit)
        {
            try
            {
                ...
                {
                    POSPrinterUtils.Instance.PrintIO_M002_M003(true, FXConsts.RECEIPT_NAME_POS_IO_M002, basketHeader, basketMiddleDeposit);
                }
            }
            catch (Exception ex)
            {
        ...
        /// <returns>메세지</returns>
        private string SetMsg(int iCnt, string strGubun)
        {
            return string.Format(WSWD.WmallPos.FX.Shared.ConfigData.Current.SysMessage.GetMessage("00387"), iCnt, strGubun);;
        }
        ...
        private int GetMath(string data)
        {
            ...
            if (data != null && data.ToString() != "" && data.ToString() != "0")
            {
                if (int.TryParse(data.ToString(), out iReturn))
                {
                    iReturn = TypeHelper.RoundDown32(iReturn);  //원단위 절사
                }
            }
