...
namespace WSWD.WmallPos.POS.FX.Win.Data
{
    public class MainMenuPresenter : IMainMenuPresenter
    {
        ...
        public MenuData[] GetChildMenus(MenuData menu)
        {
            if (menu == null)
            {
                return m_allMenus.Where(p => p.PMenuId == string.Empty && p.Visible).ToArray();
            }

            return m_allMenus.Where(p => p.PMenuId == menu.MenuId && p.Visible).ToArray();
        }
        ...
        private void LoadMenus(XmlNode node, MenuData parentMenu)
        {
            ...
            foreach (XmlNode n in nodes)
            {
                ...
                {
                    string c = n.Attributes["visible"].Value.ToLower();
                    mr.Visible = !"false".Equals(c);
                }

                if (n.Attributes["role"] != null)
                {
                    string t = n.Attributes["role"].Value;
                    mr.IsAdminMenu = "admin".Equals(t.ToLower());
                }

                // validator on click
                if (n.Attributes["validator"] != null)
                {
                    string v = n.Attributes["validator"].Value;
                    mr.ValidatorClass = v;
                }
